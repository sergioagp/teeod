...
source [file join [file dirname [file dirname [info script]]] gui/CORTEXM1_AXI_v1_1.gtcl]
...
proc init_gui { IPINST } {
  ...
  set_property tooltip {Number of interrupts.  Valid sizes of 1,8,16 & 32.  Unused IRQ should be tied low.} ${NUM_IRQ}
...
proc update_PARAM_VALUE.SMALL_DEBUG { PARAM_VALUE.SMALL_DEBUG PARAM_VALUE.DEBUG_SEL } {
	# Procedure called to update SMALL_DEBUG when any of the dependent parameters in the arguments change
	
	set SMALL_DEBUG ${PARAM_VALUE.SMALL_DEBUG}
	set DEBUG_SEL ${PARAM_VALUE.DEBUG_SEL}
	set values(DEBUG_SEL) [get_property value $DEBUG_SEL]
	if { [gen_USERPARAMETER_SMALL_DEBUG_ENABLEMENT $values(DEBUG_SEL)] } {
		set_property enabled true $SMALL_DEBUG
	} else {
		set_property enabled false $SMALL_DEBUG
	}
}
